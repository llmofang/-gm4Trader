...
* 修改交易员分配信息
    ...
    `url:` /fundaccount/change?account_name=xxx&stock_code=xxx&stock_name=xxx&stock_count=xxx&username=xxx&available_count=xxx&status=xxxqid=123  (除username外 其他为可选, 当stock_count修改后比已分配的总数小时, 已分配的数据将会被清空, 需重新分配)
...
* 查询交易员股票分配
    ...
    `url:` /fundaccount/search?account_name=xxxusername=xxx&qid=123  (只填account_name表示查询该账号下的所有分配信息, 只填username表示查询所有账号下的该用户分配信息, 都填表示查询指定account_name下的username的分配信息)

    `success_response:` {"Counter":1,"Data":{"result":["accountname":"xxx"...]},"Err":0,"Qid":"123"}  (data 返回所有字段 json)
